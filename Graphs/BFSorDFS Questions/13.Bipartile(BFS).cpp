...
class Solution {
    bool check(int start, vector<vector<int>>& graph, vector<int> &color)
    {
        ...
        while(!q.empty())
        {
            ...
            for(auto it : graph[node]) // traverse neighbours
            {
                if(color[it] == -1) // if neighbour not colored
                {
                    color[it] = !color[node]; // assign opposite color
                    q.push(it); // push neighbour
                }
                else if(color[it] == color[node]) // if same color conflict
                {
                    return false; // graph not bipartite
                }
            }
        }
        return true; // valid bipartite component
    }
...
public:
    bool isBipartite(vector<vector<int>>& graph) {
        ...
        vector<int> color(V, -1); // initialize color array

        for(int i = 0; i < V; i++) // check all components
        {
            if(color[i] == -1) // if node not colored
            {
                if(check(i, graph, color) == false) // run BFS check
...
int main()
{
    ...
    bool ans = obj.isBipartite(graph); // call function
