...
class Solution {
    int helper(vector<int>& height, vector<int>& dp, int ind) {
        if (ind == 0) return 0;

        if (dp[ind] != -1) return dp[ind];

        int left = helper(height, dp, ind - 1)
                   + abs(height[ind - 1] - height[ind]);

        int right = INT_MAX;
        if (ind > 1) {
            right = helper(height, dp, ind - 2)
                    + abs(height[ind - 2] - height[ind]);
        }
