...
class Solution {
    int helper(int i, int j, vector<vector<int>>& dp) {        // recursive helper to count paths to cell (i, j)
        if(i == 0 && j == 0) return 1;                          // base case: starting cell has one path
        if(i < 0 || j < 0) return 0;                            // out of grid means no path

        if(dp[i][j] != -1) return dp[i][j];                    // return stored result if already computed

        int up = helper(i-1, j, dp);                            // number of paths from upper cell
        int left = helper(i, j-1, dp);                          // number of paths from left cell
...
public:
    int uniquePaths(int m, int n) {
        ...
        return helper(m-1, n-1, dp);                            // calculate paths to bottom-right cell
    }
};
